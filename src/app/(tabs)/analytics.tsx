...
export default function AnalyticsScreen() {
  ...
  if (loading) return <FullScreenLoader title="분석하는 중" />;
  ...
  return (
    <View className="flex-1" style={{ backgroundColor: scheme.background }}>
      ...
      >
        <PageHeader title="애널리틱스" subtitle="여행별로 나눠서 봐요" />
...
function periodLabel(trip: Trip): string {
  if (!trip.startDate || !trip.endDate) return '기간 미설정';
  return `${trip.startDate} – ${trip.endDate}`;
}

function statusLabel(trip: Trip, isActive: boolean): string | null {
  const todayKey = localDateKey(Date.now());
  if (isActive && trip.startDate && trip.startDate <= todayKey) return '여행 중';
  if (trip.startDate && trip.startDate > todayKey) return '예정';
  return null;
}
...
function TripCard({
  ...
  const { scheme } = useTheme();
  const image = HERO_IMAGES[trip.destinationCurrency];
  const status = statusLabel(trip, isActive);

  const content = (
    <View className="gap-3 p-5">
      <View className="flex-row items-center justify-between">
        <Text className="text-xs font-bold text-white/70">
          {flagOfCurrency(trip.destinationCurrency)}{' '}
          {countryNameOfCurrency(trip.destinationCurrency)}
        </Text>
        {status ? (
      ...
      <Text className="text-xl font-black text-white">{periodLabel(trip)}</Text>

      <View className="flex-row items-end justify-between">
        <View>
          <Text className="text-[11px] font-semibold text-white/60">총 지출</Text>
          <Text className="mt-0.5 text-lg font-black text-white">
            {formatMoney(total, trip.baseCurrency)}
          </Text>
        </View>
        <Text className="text-xs font-bold text-white/70">기록 {count}건</Text>
      </View>
    </View>
  );
  ...
  return (
    <Pressable
      accessibilityRole="button"
      accessibilityLabel={`${countryNameOfCurrency(trip.destinationCurrency)} 여행 상세 분석`}
      onPress={onPress}
